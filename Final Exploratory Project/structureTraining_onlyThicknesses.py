### Program used to begin exploring training for the device strucutre layer thicknesses.
...
def main(args):
    ...
    for i in range(epochs): # Number of epochs
        print(i)
        torch.manual_seed(i) # seed entries for ease of debugging
        ...
        thicknesses_updated = phcore.dev_structure_thickness - phcore.dev_structure_thickness.grad*lr
        print("Updated thicknesses: ", thicknesses_updated)
        thicknesses_updated = torch.tensor(thicknesses_updated, requires_grad=True)
        
        print("Gradients for Thickness: \n", phcore.dev_structure_thickness.grad)
    ...
    plt.plot(losses) 
    plt.title(f'Loss Curve for Learned Thicknesses')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()

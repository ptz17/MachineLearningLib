### Program used to begin exploring training for the both device strucutre characteristics: layering and thicknesses.
import torch
...
def main(args):
    ...
    for i in range(epochs): # Number of epochs
        ...
        materials_oneHot_updated = phcore.oneHot - phcore.oneHot.grad*mats_lr
        # print("Updated materials: ", materials_oneHot_updated)
        thicknesses_updated = phcore.dev_structure_thickness - phcore.dev_structure_thickness.grad*thick_lr
        # print("Updated thicknesses: ", thicknesses_updated)

        thicknesses_updated = torch.tensor(thicknesses_updated, requires_grad=True)
    ...
    plt.plot(losses) 
    plt.title(f'Loss Curve for Learned Materials and Thicknesses')
    plt.xlabel('Epoch')
    plt.ylabel('Loss')
    plt.show()

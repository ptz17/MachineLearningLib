    ...
    for epoch in range(1, T+1):
        ...
        for i in range(len(shuffled_data)):
            ...
            for k in range(len(C)):
                y_pred = y_pred + C[k]*np.sign(np.sum(np.multiply(w[k], sample)))
            y_pred = np.sign(y_pred)
            y_pred = -1 if y_pred == 0.0 else y_pred

            # If y_i*w^T*x_i <= 0, update w, m, and Cm 
            if (label*np.matmul(w[m],sample) <= 0):  
                w.append(w[m] + lr*label*sample)
                m = m + 1
                C.append(1)
            else:
                C[m] = C[m] + 1
        ...
        for i in range(len(test_data)):
            ...
            y_pred = 0
            for k in range(len(C)):
                y_pred = y_pred + C[k]*np.sign(np.sum(np.multiply(w[k], example)))
            y_pred = np.sign(y_pred)
            y_pred = -1 if y_pred == 0.0 else y_pred
